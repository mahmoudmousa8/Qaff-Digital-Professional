...
REPORT_PATH = BASE_DIR / "hours_report.json"
def get_unique_excel_path() -> Path:
    from datetime import datetime
    now_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_name = f"youtube_hours_report_{now_str}"
    ext = ".xlsx"
...
def print_header() -> None:
    ...
    console.print("YouTube Studio Monetization Scraper", "bold cyan")
...
def scrape_channel_handles(page: Page) -> list[str]:
    ...
    for index in range(final_count):
        for handle in handles_from_item(items.nth(index)):
            key = handle.casefold()
            if key not in seen:
                seen.add(key)
                handles.append(handle)
                console.print(f"  found: {handle}")

    console.print(f"- Scraped {len(handles)} handles.")
    console.print()
    return handles
...
def metrics_are_ready(page: Page, timeout_ms: int) -> bool:
    try:
        page.locator("div.subscriber-progress #subscriber-count").first.wait_for(
            state="attached",
            timeout=timeout_ms,
        )
        page.locator("div.watch-progress #watch-count").first.wait_for(
            state="attached",
            timeout=timeout_ms,
        )
        return True
...
def dismiss_ok_popup_if_present(page: Page) -> None:
    """Dismiss an OK dialog that may appear on the monetization page."""
    try:
        xpath = (
            "//button[@aria-label='حسنًا' or normalize-space()='حسنًا'"
            " or @aria-label='OK' or normalize-space()='OK']"
            " | //ytcp-button[.//div[normalize-space()='حسنًا' or normalize-space()='OK']]//button"
        )
        btn = page.locator(f"xpath={xpath}").first
        btn.wait_for(state="attached", timeout=4000)
        console.print("- OK popup detected. Dismissing...")
        btn.evaluate("node => node.click()")
        page.wait_for_timeout(500)
    except Exception:
        pass
...
def open_monetization_overview(page: Page) -> None:
    console.print(f"- Waiting 2 seconds before clicking monetization tab...")
    page.wait_for_timeout(1000)
    
    # Just in case a delayed popup appears before clicking Earn
    dismiss_studio_welcome_if_present(page)
    
    xpath = "//tp-yt-paper-icon-item[.//div[contains(@class,'nav-item-text') and (normalize-space()='Earn' or normalize-space()='تحقيق الربح')]]"
    
    try:
        earn_btn = page.locator(xpath).first
        earn_btn.wait_for(state="visible", timeout=DEFAULT_TIMEOUT)
        earn_btn.scroll_into_view_if_needed()
        console.print(f"- Clicking 'Earn' (تحقيق الربح) menu item...")
        earn_btn.click(timeout=DEFAULT_TIMEOUT)
    except Exception as exc:
        raise StepFailure("monetization_tab_not_found", f"Failed to click the Earn tab: {exc}")

    # Give it a moment to stabilize, dismiss any OK popup, then scroll to trigger loading
    page.wait_for_timeout(1000)
    dismiss_ok_popup_if_present(page)
    page.evaluate("window.scrollBy(0, 400)")
    page.wait_for_timeout(1000)

    if metrics_are_ready(page, 5000):
        return

    console.print("- Refreshing overview page once to load Studio data...")
    page.reload(wait_until="domcontentloaded", timeout=DEFAULT_TIMEOUT)
    detect_and_wait_for_security_challenge(page)
    dismiss_ok_popup_if_present(page)
    page.evaluate("window.scrollBy(0, 400)")

    if not metrics_are_ready(page, DEFAULT_TIMEOUT):
        raise StepFailure("monetization_data_not_found", "Monetization metrics did not load after refresh.")


def parse_number(text: str) -> int:
    # Translate Arabic/Eastern digits to standard Western digits
    normalized = (text or "").translate(ARABIC_DIGIT_TRANSLATION)
    # Find all digits in the string and join them
    digits = "".join(re.findall(r"\d", normalized))
    if not digits:
        raise ValueError(f"No digits found in text: {text!r}")
    return int(digits)


def metric_number(page: Page, selector: str, label: str) -> Any:
    # Try multiple times to wait for the number to appear
    last_text = ""
    for _ in range(15):
        try:
            ...
            if loc.count() > 0:
                ...
                if text:
                    ...
                    try:
                        ...
                        # If the text explicitly says "ما مِن مشترِكين" or similar, we can just return it
                        if "ما مِن مشترِكين" in text or "No subscribers" in text:
                            return text
        except Exception:
            pass
        page.wait_for_timeout(1000)

    # Return whatever text we found, or "Not Found" if it didn't appear at all.
    # This ensures we don't fail the whole script just for a missing/unparseable number.
    if last_text:
        return last_text
    return "Not Found"


def extract_metrics(page: Page) -> dict[str, Any]:
    return {
        "subscribers": metric_number(page, "div.subscriber-progress #subscriber-count", "subscribers"),
        "watch_hours": metric_number(page, "div.watch-progress #watch-count", "watch hours"),
    }
...
def process_channel(context: BrowserContext, handle: str, index: int, total: int) -> dict[str, Any]:
    ...
    try:
        ...
        open_monetization_overview(page)
        ...
        console.print(
            ...
            f"watch_hours={metrics['watch_hours']}"
        )
